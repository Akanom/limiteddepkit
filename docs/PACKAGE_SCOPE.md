...
- random-effects and dynamic random-effects Ordered Logit; and
...
| Poisson, NB2, zero-inflated Poisson, and hurdle Poisson | The response is a non-negative integer, with additional zero mechanisms where applicable |
| Tobit, truncated Gaussian, and interval regression | The observation rule censors, truncates, or coarsens a continuous latent response |
| Gaussian sample selection | A binary selection rule determines whether the continuous outcome is observed |
| Discrete-time, Exponential, Weibull, and Gamma duration | Duration is non-negative and commonly right-censored; these remain experimental because survival workflows need additional contracts |
