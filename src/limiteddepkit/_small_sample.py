"""Implementations for small-sample and separation-resistant estimators.
...
def _stationarity_limit(
    ...
    *,
    ...
    """Return a user-sensitive but independently capped score threshold."""
    return max(min(multiplier * float(tolerance), ceiling), floor)
...
class FirthBinaryLogitResult(_PenalizedBinaryLogitResult):
    ...
    bias-reduced estimate. :meth:`conf_int` defaults to profile
    penalized-likelihood intervals; pass ``method="wald"`` to request the
    symmetric covariance-based approximation.
    """

    jeffreys_penalty: float
    step_halvings: int
    _profile_design: np.ndarray = field(repr=False, compare=False)
    _profile_outcomes: np.ndarray = field(repr=False, compare=False)
    _profile_maxiter: int = field(repr=False, compare=False)
    _profile_tolerance: float = field(repr=False, compare=False)

    def profile_penalized_loglike(
        self,
        parameter: str | int,
        ...
        *,
        ...
        """Evaluate the constrained profile penalized log likelihood.

        The named coefficient is fixed at each supplied value while every
        other coefficient is re-optimized. This is the same profile used by
        :meth:`conf_int` and is useful for diagnostics or plotting.
        """

        parameter_index, parameter_name = _resolve_profile_parameter(
            parameter, self.feature_names
        )
        value_array = np.atleast_1d(np.asarray(values, dtype=float))
        if value_array.ndim != 1 or not np.isfinite(value_array).all():
            raise ValueError("values must be a finite scalar or one-dimensional sequence.")
        profile_maxiter, profile_tolerance = _profile_controls(
            maxiter,
            tolerance,
            default_maxiter=self._profile_maxiter,
            default_tolerance=self._profile_tolerance,
        )
        fitted = self.params.to_numpy(dtype=float)
        likelihoods = [
            _constrained_firth_penalized_loglike(
        ...
        return pd.Series(
            likelihoods,
            index=pd.Index(value_array, name=parameter_name),
            name="profile_penalized_loglike",
        )
    ...
    def conf_int(
        ...
        *,
        ...
        """Return profile penalized-likelihood or approximate Wald intervals.

        Profile bounds invert a one-degree-of-freedom penalized likelihood-
        ratio test while re-optimizing all nuisance coefficients. They are the
        default because small samples and separation commonly make the
        ...
        if normalized_method == "wald":
            return super().conf_int(level)
        ...
        fitted = self.params.to_numpy(dtype=float)
        critical = float(chi2.ppf(level, df=1))
        intervals = np.empty((len(fitted), 2), dtype=float)
        for parameter_index in range(len(fitted)):
            standard_error = float(self.standard_errors.iloc[parameter_index])
            intervals[parameter_index] = [
                _firth_profile_bound(
                    self._profile_design,
                    self._profile_outcomes,
                    fitted,
                    self.penalized_loglike,
                    parameter_index,
                    direction,
        ...
        return pd.DataFrame(intervals, index=self.params.index, columns=["lower", "upper"])
...
def _resolve_profile_parameter(
    parameter: str | int, feature_names: tuple[str, ...]
) -> tuple[int, str]:
    if isinstance(parameter, str):
        if parameter not in feature_names:
            raise ValueError(
                f"Unknown parameter {parameter!r}; expected one of {list(feature_names)!r}."
            )
        return feature_names.index(parameter), parameter
    if isinstance(parameter, bool) or not isinstance(parameter, (int, np.integer)):
        raise TypeError("parameter must be a fitted feature name or integer position.")
    index = int(parameter)
    if index < 0 or index >= len(feature_names):
        raise IndexError(
            f"parameter index {index} is out of range for {len(feature_names)} coefficients."
        )
    return index, feature_names[index]
...
def _profile_controls(
    ...
    *,
    ...
    resolved_maxiter = default_maxiter if maxiter is None else maxiter
    resolved_tolerance = default_tolerance if tolerance is None else tolerance
    return _validate_optimizer_options(resolved_maxiter, resolved_tolerance)


def _constrained_firth_penalized_loglike(
    design: np.ndarray,
    outcomes: np.ndarray,
    fitted: np.ndarray,
    parameter_index: int,
    ...
    *,
    ...
    """Maximize the Firth objective with one coefficient held fixed."""

    free_indices = np.delete(np.arange(fitted.size), parameter_index)

    def assemble(free_parameters: np.ndarray) -> np.ndarray:
        coefficients = fitted.copy()
        coefficients[parameter_index] = fixed_value
        coefficients[free_indices] = free_parameters
        return coefficients

    if free_indices.size == 0:
        return float(_firth_components(design, outcomes, assemble(np.empty(0)))[0])

    nuisance_stationarity_limit = _stationarity_limit(
        tolerance,
        multiplier=100.0,
        floor=1e-7,
        ceiling=1e-5,
    ...
    # L-BFGS-B may stop on relative objective change while the nuisance score
    # is still materially nonzero. That is especially common when predictors
    # have different scales, and it can make an otherwise regular profile
    # confidence interval fail during bracketing. Use the same adjusted-score
    # Fisher-scoring direction as the full Firth fit, restricted to the free
    # coordinates, and retain an independently capped score certificate.
    scoring_target = min(
        nuisance_stationarity_limit,
        _stationarity_limit(
            tolerance,
            multiplier=10.0,
            floor=1e-8,
    ...
    constrained = assemble(fitted[free_indices])
    nuisance_score_norm = np.inf
    failure_reason = "iteration limit reached"

    for _ in range(maxiter + 1):
        try:
            penalized_loglike, _, adjusted_score, information, _ = _firth_components(
                design, outcomes, constrained
            )
        except (np.linalg.LinAlgError, RuntimeError) as error:
            failure_reason = str(error)
            break
        nuisance_score = adjusted_score[free_indices]
        nuisance_score_norm = float(np.max(np.abs(nuisance_score)))
        if (
            np.isfinite(penalized_loglike)
            and np.isfinite(nuisance_score_norm)
            and nuisance_score_norm <= scoring_target
        ):
            return float(penalized_loglike)

        nuisance_information = information[np.ix_(free_indices, free_indices)]
        try:
            scoring_step = np.linalg.solve(nuisance_information, nuisance_score)
        except np.linalg.LinAlgError:
            failure_reason = "nuisance Fisher information is singular"
            break
        if not np.isfinite(scoring_step).all():
            failure_reason = "nuisance Fisher-scoring step is non-finite"
            break
        largest_step = float(np.max(np.abs(scoring_step)))
        if largest_step > 5.0:
            scoring_step *= 5.0 / largest_step

        step_scale = 1.0
        accepted = False
        for _ in range(51):
            candidate = constrained.copy()
            candidate[free_indices] += step_scale * scoring_step
            try:
                candidate_loglike = _firth_components(
                    design, outcomes, candidate
                )[0]
            except (np.linalg.LinAlgError, RuntimeError):
                candidate_loglike = -np.inf
            if (
                np.isfinite(candidate_loglike)
                and candidate_loglike >= penalized_loglike - 1e-12
            ):
            ...
            step_scale *= 0.5
        if not accepted:
            failure_reason = "monotone nuisance step halving failed"
            break

    raise RuntimeError(
        "Firth profile nuisance optimization failed at "
        f"coefficient {parameter_index}={fixed_value:.8g}: "
        f"{failure_reason}; nuisance score norm={nuisance_score_norm:.6g} "
        f"(required <= {scoring_target:.6g})."
    )


def _firth_profile_bound(
    design: np.ndarray,
    outcomes: np.ndarray,
    fitted: np.ndarray,
    maximum_penalized_loglike: float,
    parameter_index: int,
    direction: int,
    *,
    ...
    """Find one profile penalized-likelihood confidence bound."""

    center = float(fitted[parameter_index])

    def profile_deviance(value: float) -> float:
        constrained_loglike = _constrained_firth_penalized_loglike(
            design,
            outcomes,
            fitted,
            parameter_index,
        ...
        return 2.0 * (maximum_penalized_loglike - constrained_loglike) - critical

    center_value = profile_deviance(center)
    if center_value >= 0.0:
        raise RuntimeError(
            "Firth profile likelihood at the fitted coefficient is not the constrained maximum."
        )

    step = max(float(standard_error), 0.25)
    outer = center + direction * step
    for _ in range(40):
        outer_value = profile_deviance(outer)
        if outer_value >= 0.0:
            lower, upper = sorted((center, outer))
            return float(
                brentq(
                    profile_deviance,
                    lower,
                    upper,
                    xtol=max(tolerance, 1e-10),
                    maxiter=maxiter,
                )
            )
        step *= 2.0
        outer = center + direction * step
    raise RuntimeError(
        "Could not bracket a finite Firth profile confidence bound for "
        f"coefficient {parameter_index} in direction {direction}."
    )
...
class FirthBinaryLogit:
    ...
    def fit(
        ...
        *,
        ...
        stationarity_limit = _stationarity_limit(tolerance)
        ...
        for iteration in range(1, maxiter + 1):
            ...
            if score_norm <= stationarity_limit:
                converged = True
            ...
            if (
                ...
                and new_score_norm <= stationarity_limit
            ):
                converged = True
                break
        ...
        score_norm = float(np.max(np.abs(adjusted_score)))
        converged = bool(score_norm <= stationarity_limit)
        ...
        return FirthBinaryLogitResult(
            ...
            inference_note=(
                ...
                "bias-reduced estimate is reported in summary_frame; conf_int defaults to "
                "profile penalized-likelihood intervals."
            ),
            jeffreys_penalty=jeffreys_penalty,
            step_halvings=total_step_halvings,
            _profile_design=design.copy(),
            _profile_outcomes=outcomes.copy(),
            _profile_maxiter=maxiter,
            _profile_tolerance=max(tolerance, 1e-8),
        )
